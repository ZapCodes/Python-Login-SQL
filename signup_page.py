...
class SignUp:
    def __init__(self, root):
        ...
        frame = Frame(self.window, bg="darkgray")
        frame.place(x=350,y=100,width=500,height=550)

        title1 = Label(frame, text="Sign Up", font=("times new roman",25,"bold"),bg="darkgray").place(x=20, y=10)
        title2 = Label(frame, text="Join with us", font=("times new roman",13),bg="darkgray", fg="darkgray").place(x=20, y=50)

        f_name = Label(frame, text="First name", font=("helvetica",15,"bold"),bg="darkgray").place(x=20, y=100)
        l_name = Label(frame, text="Last name", font=("helvetica",15,"bold"),bg="darkgray").place(x=240, y=100)
        ...
        email = Label(frame, text="Email", font=("helvetica",15,"bold"),bg="darkgray").place(x=20, y=180)

        self.email_txt = Entry(frame,font=("arial"))
        self.email_txt.place(x=20, y=210, width=420)

        sec_question = Label(frame, text="Security questions", font=("helvetica",15,"bold"),bg="darkgray").place(x=20, y=260)
        answer = Label(frame, text="Answer", font=("helvetica",15,"bold"),bg="darkgray").place(x=240, y=260)
        ...
        password =  Label(frame, text="New password", font=("helvetica",15,"bold"),bg="darkgray").place(x=20, y=340)

        self.password_txt = Entry(frame,font=("arial"))
        self.password_txt.place(x=20, y=370, width=420)

        self.terms = IntVar()
        terms_and_con = Checkbutton(frame,text="I Agree The Terms & Conditions",variable=self.terms,onvalue=1,offvalue=0,bg="darkgray",font=("times new roman",12)).place(x=20,y=420)
        self.signup = Button(frame,text="Sign Up",command=self.signup_func,font=("times new roman",18, "bold"),bd=0,cursor="hand2",bg="green2",fg="blue").place(x=120,y=470,width=250)
    ...
    def signup_func(self,):
        if self.fname_txt.get()=="" or self.lname_txt.get()=="" or self.email_txt.get()=="" or self.questions.get()=="Select" or self.answer_txt.get()=="" or self.password_txt.get() == "":
            ...
            try:
                host = ('192.168.0.56')
                connection = pymysql.connect(host='192.168.0.56', user='fxserver', password='fxserver', database='student_database')
